...
Button(frame,width=39,pady=7, text='Giriş Yap ',command=giris_yap,cursor='hand2',bg='#57a1f8',fg='white',border=2).place(x=35,y=204)
...
def sign_up():
    ...
    def geri_don():
        sign_in = Toplevel()
        sign_in.title('Login')
        sign_in.geometry('925x500+300+200')
        sign_in.configure(bg="#fff")
        sign_in.resizable(False, False)

        img = PhotoImage(file='giris1.png').subsample(4, 4)
        Label(sign_in, image=img, bg='white').place(x=80, y=50)

        frame = Frame(sign_in, width=350, height=350, bg="white")
        frame.place(x=480, y=70)
        ...
        Button(frame, width=39, pady=7, text='Giriş Yap ', cursor='hand2', bg='#57a1f8', fg='white', border=2).place(
            x=35,
            y=204)
        label = Label(frame, text="Hesabınız yok mu ?", fg='black', bg='white', font=('Microsoft YaHei UI Light', 9))
        label.place(x=75, y=270)

        sign_up = Button(frame, width=6, text="Kayıt Ol",command=kayit_ol, border=2, bg='white', cursor='hand2',
                         fg='#57a1f8')
    ...
    geri_don_button = ctk.CTkButton(frame, text="Geri Dön", command=geri_don, corner_radius=10, fg_color="red",
                                    width=200)
    geri_don_button.pack(pady=10)

    kayit_ol_penceresi.mainloop()

sign_up=Button(frame,width=6,text="Kayıt Ol",command=sign_up,border=2,bg='white',cursor='hand2',fg='#57a1f8')
sign_up.place(x=215,y=270)
...
def anasayfa():
    ...
    anasayfa_pencere = ctk.CTkToplevel()

    anasayfa_pencere.geometry("850x750")
    anasayfa_pencere.title("Restoran Adisyon Sistemi")



    ust_cerceve = ctk.CTkFrame(anasayfa_pencere, height=50, corner_radius=10)
    ust_cerceve.pack(side="top", fill="x", pady=10)

    alt_cerceve = ctk.CTkFrame(anasayfa_pencere, height=50, corner_radius=10)
    alt_cerceve.pack(side="bottom", fill="x", pady=10)

    sol_cerceve = ctk.CTkFrame(anasayfa_pencere, width=600, height=700, corner_radius=10)
    sol_cerceve.pack(side="left", fill="y", padx=60, pady=0)

    sag_cerceve = ctk.CTkFrame(anasayfa_pencere, width=400, corner_radius=10)
    ...
    def hesap_makinesi_tuslari():
        ...
        for satir in tuslar:
            ...
            cerceve.pack(fill="x")
            for tus in satir:
                ctk.CTkButton(cerceve, text=tus, command=lambda t=tus: hesap_makinesi_buton_tikla(t)).pack(side="left",
                                                                                                           expand=True,
                                                                                                           fill="x",
                                                                                                           padx=2,
                                                                                                           pady=2)
